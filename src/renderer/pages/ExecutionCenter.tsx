...
import { CheckOutlined, CloseOutlined, PlayCircleOutlined, StopOutlined, LockOutlined, SafetyOutlined, ThunderboltOutlined } from '@ant-design/icons';
...
const ExecutionCenter = () => {
  ...
  const approvalColumns = [
    { title: 'Command', dataIndex: 'command_text', key: 'command_text', ellipsis: true, width: 400,
      render: (v: string) => <code style={{ color: '#39ff14', fontSize: 12 }}>{v}</code> },
    ...
    { title: 'Time', dataIndex: 'created_at', key: 'created_at', render: (v: string) => <span style={{ color: '#8888aa' }}>{new Date(v).toLocaleTimeString()}</span> },
    {
  ...
  const tabItems = [
    {
      ...
      children: (
        <div>
          ...
          {/* Task Orchestrator */}
          <Card title={<span><ThunderboltOutlined style={{ color: '#ff6b00' }} /> Task Orchestrator</span>} size="small" style={{ marginBottom: 16 }}
          ...
          <Typography.Title level={5}>
            <SafetyOutlined style={{ color: '#00f0ff' }} /> {t('execution.approvalQueue')}
          ...
          {/* Execution Stream */}
          <Divider />
          <Typography.Title level={5}><ThunderboltOutlined style={{ color: '#39ff14' }} /> Execution Stream</Typography.Title>
          <Card size="small" className="terminal-block" style={{ minHeight: 200 }}>
          ...
          {/* Scope Gate Status */}
          <Card size="small" style={{ marginTop: 16 }} title={<span><LockOutlined style={{ color: '#ff073a' }} /> Scope Gate Status</span>}>
            <Space>
              <Tag color="green" className="neon-pulse">Active</Tag>
  ...
  return (
    <div>
      <Typography.Title level={3} className="neon-title">{t('execution.title')}</Typography.Title>
      <Tabs items={tabItems} />
